...
function loadWhichBrowser(cb) {
	...
	function load() {
		...
		const src = '/api.whichbrowser.net/rel/detect.js?' + p.join('&');
		script.src = '.' + src;
		document.getElementsByTagName('head')[0].appendChild(script);
		const checker = ()=>typeof WhichBrowser0 != 'undefined'
		void wait(checker,()=>callback(WhichBrowser0));

		{
			const script = document.createElement('script');
			script.src = 'https://' + src;
			document.getElementsByTagName('head')[0].appendChild(script);
			const checker = () => typeof WhichBrowser != 'undefined'
			void wait(checker,()=>callback(WhichBrowser));
		}
	}

	async function wait(checker,callback) {
...
Confirm.prototype = {
	initialize: function (parent, options) {
		...
		if(Browsers=='an unknown browser'){
			this.useragent.innerHTML = '';
		}else{
